...
Two methods based on graph similarity have been implemented in `r
Biocpkg("GOstats")` they are `simUI` and `simLP`. The first uses the number of
nodes that the two graphs have in common divided by the number of nodes in the
union of the two node sets. Hence these similarities are bounded between 0 and 1
...
You should refer to the documentation for the `r Biocpkg("graph")` and `r
Biocpkg("RBGL")` packages for details on how to manipulate *graph* instances.
