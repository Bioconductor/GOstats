...
\texttt{GO:0003700} there are \Sexpr{length(alltfprobes)} probes.
...
ones. The most specific terms are at the top of the graph
...
approaches that could be used are Spearman's correlation, that use by
...
And we can see that GADD45A has much higher counts than the rest.
Additionally MYC, MPO and L0C57228 each have several. Very little is
currently known about L0C57228 and so we concentrate our examination
on the other three genes. The evidence here suggests that, perhaps,
the expression patterns of these three different transcription factors
are substantially different in the two phenotypes we are studying.
...
in the NEG group. For MYC the means were \Sexpr{round(mean(nMYC),1)} for
NEG and \Sexpr{round(mean(bMYC),1)} for BCR/ABL, for MPO they were
\Sexpr{round(mean(nMPO),1)} for NEG and \Sexpr{round(mean(bMPO),1)} for
BCR/ABL and for GADD45A the means were \Sexpr{round(mean(nGADD),1)} for
NEG and \Sexpr{round(mean(bGADD),1)} for BCR/ABL. It is rather
...
that there are positive correlations between MYC and KIAA0540 and as
well between KIAA0540 and HMG20B, but that for HMG20B and MPO the
...
suggestive of transcriptional inhibition. But the reader is cautioned
that these are stable values averaged over many cells and are not in
any sense a time-course so direct relationships are very difficult to
establish.
...
about \Sexpr{round(mean(c(MYCMPO.Bwts,MYCMPO.Nwts,
  MYCGADD45A.Bwts,MYCGADD45A.Nwts)),1)} but the number of edges is
...
between MYC and MPO in each of the two phenotypes.
