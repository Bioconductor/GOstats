...
<<GOcharacteristics, echo=FALSE, results=tex>>=
 ...
 goCats <- sapply(goTerms, Category)
...
<<tfGplot, fig=TRUE, echo=FALSE, results=hide>>=
   nL <- nodes(tfG); nLterm = getGOTerm(nL)[["MF"]]
   names(nLterm) = nL
...
(R \Robject{environment}) named \Robject{GOLOCUSID}, which maps from GO
terms to LocusLink identifiers. These are only the set of most
specific mappings. To get all LocusLink IDs associated with a specific
GO term use \Robject{GOALLOCUSID} instead. A histogram of the logarithm of the
...
nL <- rep("", length(nodes(gGO)))
 ...
 gNsLL <- unique(unlist(mget(gNsel,
...
from the observation that the proportion of values that were larger
...
about
\Sexpr{round(mean(c(MYCMPO.Bwts,MYCMPO.Nwts, MYCGADD45A.Bwts,MYCGADD45A.Nwts)),1)} but the number of edges is
