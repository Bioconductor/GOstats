...
entrezIds <- mget(geneNames(bcrAblOrNeg), envir=hgu95av2LOCUSID)
...
gNll = as.character(unlist(mget(deGenes, hgu95av2LOCUSID)))
...
deGenes_entrez <- mget(deGenes, env=hgu95av2LOCUSID, ifnotfound=NA)
stopifnot(!any(duplicated(deGenes_entrez)))
stopifnot(!any(is.na(deGenes_entrez)))
gGhyp <- GOHyperG(deGenes_entrez)
