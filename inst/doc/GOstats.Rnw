...
\Sexpr{length(ww)} with duplicate entries (\textit{technical
  replicates}. A visual inspection (not reported) suggested that the
correlation between these duplicate probes was quite high and so only
one of each was used in the subsequent analysis. This left us with
\Sexpr{length(oTF3)} distinct transcription factors for our study.
