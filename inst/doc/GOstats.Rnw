...
<<GOcharacteristics, echo=FALSE, results=tex>>=
 ...
 goCats <- sapply(goTerms, function(x) names(x))
...
we see that for version \Sexpr{package.description("GO")["Version"]}
...
<<tfG,results=hide, echo=FALSE>>=
 tfG <- oneGOGraph("GO:0003700", GOMFPARENTS)
 ##drop the GO node
 tfGs <- removeNode("GO:0003673", tfG)
@

\begin{figure}[htbp]
  \begin{center}
<<tfGplot, fig=TRUE, echo=FALSE, results=hide>>=
   nL <- nodes(tfG); nLterm = unlist(mget(nL, GOTERM))
   names(nLterm) = nL
   nLterm["GO:0003673"] <-  "Gene Ontology"
...
(R \Robject{environment}) named \Robject{GOGO2LL}, which maps from GO
terms to LocusLink identifiers. A histogram of the logarithm of the
number of LocusLink identifiers per GO term is given in
Figure~\ref{fig:GO2LL}.

\begin{figure}[htbp]
\begin{center}
<<GO2LL, fig=TRUE, echo=FALSE>>=
 go2LL <- contents(GOGO2LL)
 go2LLcounts <- sapply(go2LL, length)
 hist(log(go2LLcounts), main="Log Counts of LocusLink IDs per GO ID",
      xlab="")
@
\caption{LocusLink counts per GO term (log scale). \label{fig:GO2LL}.}
\end{center}
\end{figure}

For any particular microarray there may be many probes that are mapped
to the same LocusLink identifier. In some of the computations it will
...
\Sexpr{sum(slen1 > 1)} LocusLink IDs that have more than one probe set
...
There are \Sexpr{length(tfprobes)} Affymetrix probes that are
...
together with those annotated at any of the child nodes.  This is
\Sexpr{length(alltfprobes)} probes.
...
RMA~\citep{Irizarry2003}. Of particular interest is the comparison of
37 samples from patients with the BCR/ABL
fusion gene resulting from a chromosomal translocation (9;22) with
the 42 samples from the NEG group.
...
##get the gene names
ABL1 <- "1635_at"
ABL1LL <- get(ABL1, hgu95av2LOCUSID)
...
\cite{WileyEncy} and the interested reader is referred there). A
non-specific filter was used to remove genes that showed little or no
change in expression level across experiments. The resulting data set
had \Sexpr{sum(selected)} probes remaining. To select genes whose
expression values were associated with the phenotypes of interest
(BCR/ABL and NEG) we used the \Rfunction{mt.maxT} function from the
\Rpackage{multtest} package which computes a permutation based t-test
for comparing two groups.

After adjustment for multiple testing there were only
\Sexpr{sum(resT$adjp<0.05)} probes %$
(which correspond to \Sexpr{length(unique(numLT))} genes) with an
adjusted $p$-value below 0.05. Using those genes we obtain the set of
most-specific GO terms in the MF ontology that they are annotated at
and compute the induced GO graph which is rendered in
Figure~\ref{fig:GG1}. No labels have been added to the nodes in this
plot since there is not sufficient room to provide informative
ones. Notice that the most specific terms are at the top of the graph
and that arrows go from more specific nodes to less specific ones. The
node in the bottom center is the MF node. Clearly some sort of
interactivity (e.g. tooltips) would be beneficial. We will return to
this plot in the next section and use it to provide a more detailed
view of the data.

<<inducedGO, echo=FALSE, results=hide>>=
 whGO = resT$index[resT$adjp<0.05]
 gNsel <- geneNames(esetSub)[whGO]
 gGO <- makeGOGraph(gNsel, "MF", "hgu95av2")
##drop the GO root node
 gGO <- removeNode("GO:0003673", gGO)
...
must work in terms of the unique LocusLink identifiers and not the
...
functionality is provided in the function \Rfunction{GOHyperG}
...
nL <- rep("", length(nodes(gGO)))
 ...
 gNsLL <- unique(unlist(mget(gNsel, env=hgu95av2LOCUSID, ifnotfound=NA)))
...
$p$-value was less than $0.10$ or not (for those viewing this document
in black and white the nodes should be dark and light grey,
respectively). The GO terms for the terms colored red are printed
below. The relevant biology suggests that these are quite reasonable.
We note that while the smallest $p$-values are associated with nodes
that have few genes annotated at them there are some nodes with a
reasonable number of genes annotated at them (counts) and small
$p$-values.

<<GOterms, echo=FALSE, results=tex>>=
gg = gGhyp.pv[gGhyp.pv < 0.1]
gg = sort(gg)
gg.terms <- unlist(mget(names(gg), GOTERM))

##fix up the labels etc for printing
nms = names(gg.terms)
nms2 = gsub("^GO:", "", nms)
nms3 = gsub(".MF$", "", nms2)
ggt = unlist(gg.terms)
...
ggMat = matrix(c(nms3, ggt3, round(gg,3), gg.counts),
    byrow=FALSE, nc=4, dimnames=list(rep("", length(gg)), c("GO ID",
...
<<GOxx, fig=TRUE, echo=FALSE>>=
  plot(gGO, nodeAttrs=nA)
@
\caption{The induced GO graph colored according to unadjusted Hypergeometric
p-values \label{fig:indGG}}
\end{center}
\end{figure}
...
resTGO <- mt.maxT(exprs(esetSub[whSel,]), classlabel=cl, B=100000)
...
each group we define the distance between two genes as one minus the
Pearson correlation (other approaches such as that use by
\cite{shortestpath} or some other robust correlation estimates) We
then used an edge weight of $d(u,v) = (1-C_{u,v})^k$ with $k=1$ and
$\tau=0.6$ as the cutoff for correlations, if $C_{u,v}<\tau$ then no
edge exists (Zhou et. al used $k=6$ in their analysis and some
...
##check to see how many of these are duplicated and which ones
LL2 = getLL(oTF2, "hgu95av2")

ww = table(LL2)[table(LL2)>1]

dups = list()
for(i in 1:length(ww)) dups[[i]] = names(LL2)[LL2==names(ww)[i]]
zz = unlist(dups)
##these are all highly correlated with each other - so I will just
##use the first one of each
##note that the fourth pair looks odd - and might be worth
##investigating

##drop duplicates
oTF3 = oTF2[!duplicated(LL2)]
@

Our interest in this particular example is on transcription
factors. Hence we use the GO term \texttt{GO:0003700} which maps to
the molecular function \texttt{transcription factor activity} to
identify all genes with transcription factor activity. We used only
genes for which this was a most specific annotation and obtained
\Sexpr{length(TF2)} mappings and \Sexpr{sum(!duplicated(LLs))} unique
LocusLink ids. Of these \Sexpr{length(oTF2)} were among those probes
selected for our analysis. Of these we found that there were
\Sexpr{length(ww)} with duplicate entries. A visual inspection (not
reported) suggested that the correlation between these duplicate
probes was quite high and so only one of each was used in the
subsequent analysis. This left us with \Sexpr{length(oTF3)} distinct
transcription factors for our study.
...
Bgraph <- compCorrGraph(Bsub)
Ngraph <- compCorrGraph(Nsub)
@
...
%%FIXME: this takes way to long to run every time so these are precomputed
%%and stored - but probably some careful checking should be done
<<computeDists, hide=TRUE, echo=FALSE>>=
##Ndists <- compGdist(Ngraph, oTF3)
##save(Ndists, file="Ndists.rda")
##Bdists <- compGdist(Bgraph, oTF3)
...
%%FIXME: is the symbol for 1106_s_at really TRA@
<<reportNotConn, results=tex, echo=FALSE>>=
 ...
 dimnames(outM) = list(NULL, c("Affymetrix ID", "Symbol", "Which graph"))
 xtable(outM,
...
zero and the test statistic was \Sexpr{round(tt$estimate, 3)} with an extremely
small $p$-value. So we see that distances in the NEG graph seem to be longer
than those in the BCR/ABL. Further evidence of this difference comes
from the observation that proportion of values that were larger
in the NEG graph was \Sexpr{round(sum(ndx>0)/length(ndx), 3)}.
%$ - ESS is confused
...
<<analyseFinite, results=hide, echo=FALSE>>=
 ...
 rowSym <- unlist(mget(dNames$rowNames, hgu95av2SYMBOL, ifnotfound=NA))
...
In Figure~\ref{fig:absdist} a histogram of the absolute
value of the pairwise differences is plotted. We see a number of
interesting features. For example the large number of differences that
are small in absolute value as well as a rather long right hand
tail. We will focus our attention on those differences that are large
in absolute value. We chose a value of $2.5$ as our cut-off and found
that there were \Sexpr{sum(whOnes)} differences that were larger than
$2.5$. These corresponded to \Sexpr{length(unique(rowSym))} distinct
genes.

While all may be interesting and a particular investigator may want to
expend considerable effort in study transcription factors that are of
particular interest we will center our analysis on the set of genes
that appear most frequently in this list. The multiplicities are
reported below.
...
##found 3 genes - MYC, GADD45A and MPO
...
%%FIXME: watch out here as the facts may change as the data do
And we can see that there are three genes that have substantially
higher counts than the others, namely, MYC, MPO and GADD45A. This fact
suggests that perhaps the expression patterns of these three different
transcription factors are substantially different in the two
phenotypes we are studying.

For each of the three transcription factors we can compute the average
distance, separately within each graph, to all the other selected
genes. We find that the results are quite consistent and that in all
cases the path length is much shorter in the BCR/ABL group than it is
in the NEG group. For MYC the means were \Sexpr{round(mean(nMYC))} for
NEG and \Sexpr{round(mean(bMYC))} for BCR/ABL, for MPO they were
\Sexpr{round(mean(nMPO))} for NEG and \Sexpr{round(mean(bMPO))} for
BCR/ABL and for GADD45A the means were \Sexpr{round(mean(nGADD))} for
NEG and \Sexpr{round(mean(bGADD))} for BCR/ABL. It is rather
interesting to observe that amongst the pairwise distances that have
changed the most are those between these three specific genes.
...
in each of the two graphs. In our examples we focus on MYC and the
distances between it and MPO and GADD45A.

<<graphsandpaths, echo=FALSE, results=hide>>=

 ##a little helper function to take the names off of the sp.between
 ##paths and
 #nm2sym

 ##get the Affy IDs for the genes that connect to MYC, and are
 ##interesting
 affyN.MYC = names(colSym)[rowSym=="MYC"]

 affyMYC = names(rowSym)[rowSym=="MYC"][1]
 affyMPO = names(rowSym)[rowSym=="MPO"][1]
 affyGADD45A = names(rowSym)[rowSym=="GADD45A"][1]


 ##compute the details of the paths between MYC and the other TFs

 spB = sp.between(Bgraph, (names(rowSym)[rowSym=="MYC"])[1],
                 affyN.MYC)

 spN = sp.between(Ngraph, (names(rowSym)[rowSym=="MYC"])[1],
                  affyN.MYC)


 ##the MYC-MPO paths are then
 MYCMPO.B = spB[[paste(affyMYC, affyMPO, sep=":")]]
 MYCMPO.N = spN[[paste(affyMYC, affyMPO, sep=":")]]

 MYCGADD45A.B = spB[[paste(affyMYC, affyGADD45A, sep=":")]]
 MYCGADD45A.N = spN[[paste(affyMYC, affyGADD45A, sep=":")]]
...
both MPO and GADD45A for each of the two phenotypes, respectively
(first the paths for BCR/ABL, then for the NEG samples). The MYC to MPO
results are:
...
that there are positive correlations between MYC and EIF4G1 and as
well between EIF4G1 and HMG20B, but that for HMG20B and MPO the
correlation is negative. Positive correlations are suggestive of
shared transcriptional activity while negative correlations are
suggestive of transcriptional inhibition.
...
We do not have space to present the other pairwise scatterplots here
but readers that are making use of the compendium version of this
paper can easily explore those different plots on their own.
...
<<createGraphs, results=hide, echo=FALSE>>=
 ...
 ##get the edgeweights along a path
 getEW = function(G, path) {
   whones = match(path, nodes(G))
   if(any(is.na(whones)))
    stop("path/graph mismatch")

   eL = G@edgeL
   rval = rep(0, length(whones)-1)
   for(i in 1:(length(whones)-1))
     rval[i] = (eL[[whones[i]]]$weights)[as.character(whones[i+1])]
   names(rval) = paste(path[1:(length(path)-1)], path[2:length(path)],
        sep="->")
   rval
}

 MYCMPO.Bwts = getEW(BsG, MYCMPO.B$path)
 MYCMPO.Nwts = getEW(NsG, MYCMPO.N$path)
 mean(MYCMPO.Bwts)
 mean(MYCMPO.Nwts)

 MYCGADD45A.Bwts = getEW(BsG, MYCGADD45A.B$path)
 MYCGADD45A.Nwts = getEW(NsG, MYCGADD45A.N$path)
 mean(MYCGADD45A.Bwts)
 mean(MYCGADD45A.Nwts)

@

We notice that the path lengths for the NEG samples are longer
(involve more genes) than those for the BCR/ABL samples. We might also
want to ask whether the distances are also larger (that is that the
correlations are smaller). To do this we need to obtain the edge
weights from the respective graphs and compare them. We found that
there appeared to be no difference (all averaged around a distance of
about $0.65$) but the number of edges is quite small and one might
expect to see systematic differences if a larger study were undertaken.

We can check our results, at least to some extent, by examining
pairwise scatterplots of the gene expressions. In
Figure~\ref{fig:pairs} the genes on the path from MYC to MPO are
plotted. We see quite strong correlations along the diagonal and note
that HMG20B and MPO have a negative correlation.

\begin{figure}[htbp]
\begin{center}
<<pathPairs, fig=TRUE, echo=FALSE>>=
pairs(t(exprs(Bsub)[MYCMPO.B$path,]))
@
\caption{Pairwise scatterplots of gene expression for those genes on
the shortest path between MYC and MPO from patients with the BCR/ABL
...
that is the case here). We consider two specific paths -
...
## nA$fillcolor = gCols
 ...
 if(interactive()) {
   nA$fillcolor = bc1
   plot(BsG, "neato", nodeAttrs=nA, main="BCR/ABL: MYC")
   dev.set(dev.next())
   nA$fillcolor = nc1
   plot(NsG, "neato", nodeAttrs=nA, main="NEG: MYC")
}
 ...
 Ncols = colorPath(NsG, MYCMPO.N$path)
 Bcols = colorPath(BsG, MYCMPO.B$path)
@

\begin{figure}[htbp]
\begin{center}
<<pathinNEG, fig=TRUE, echo=FALSE>>=
 nAN = nA; nAN$fillcolor = Ncols
 plot(NsG, "neato", nodeAttrs=nAN)

@
\caption{Shortest path between MYC and MPO in the NEG
...
<<pathinBCR, fig=TRUE, echo=FALSE>>=
 nAB = nA; nAB$fillcolor = Bcols
 plot(BsG, "neato", nodeAttrs=nAB)
@
\caption{Shortest path between MYC and MPO in the BCR/ABL
  samples. \label{fig:BCRsp}.}
\end{center}
\end{figure}

We see, in Figures~\ref{fig:NEGsp} and \ref{fig:BCRsp}, the actual
shortest paths between the genes MYC and MPO. The two end points have
...
reproduced on any users machine equipped with R and the appropriate
