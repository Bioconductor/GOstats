...
%\VignetteDepends{ALL, hgu95av2, genefilter, RColorBrewer, xtable, Rgraphviz}
...
abnormality (``NEG'') or those with ``\Sexpr{subsetType}''.
...
\item[\Robject{geneIds}] A vector of gene identifiers that defines the
  ...
  package, the vector will consist of FIXME.
...
\item[\Robject{conditional}] A logical value.  If \Robject{TRUE}, the
  ...
  standard Hypergeometric test is performed.
...
params <- new("GOHyperGParams",
              ...
              annotation="hgu95av2db",
## FIXME
              datPkg=Category:::DatPkgFactory("hgu95av2db"),
              ontology="BP",
...
extract detail from a result object.
