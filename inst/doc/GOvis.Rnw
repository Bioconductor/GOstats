...
## there will soon be a better way to obtain this eg => GO map
eg2go = mget(gN, hgu95av2GO)
names(eg2go) = lls
eg2go = l2e(eg2go)

 gMF <- makeGOGraph(lls, "MF", eg2go.map=eg2go)
 gBP = makeGOGraph(lls, "BP", eg2go.map=eg2go)
 gCC = makeGOGraph(lls, "CC", eg2go.map=eg2go)
...
<<multip>>=
 ...
 ggs = lapply(igenes[hasBP], makeGOGraph, "BP", eg2go)
