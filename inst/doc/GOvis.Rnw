...
\Sexpr{packageDescription("hgu95av2")[["Version"]]} of the
\Rpackage{hgu95av2} data package and on
\Sexpr{packageDescription("GO")[["Version"]]} of the \Rpackage{GO}
...
<<inducedGO, echo=FALSE, results=hide>>=
 ...
 gMF <- makeGOGraph(lls, "MF")
 gBP = makeGOGraph(lls, "BP")
 gCC = makeGOGraph(lls, "CC")
...
We can make use of the \Rfunction{imageMap} function from the
\Rpackage{prada} package to provide tool tips for our graph.
...
##to get the right user coords on the jpg file...
 ...
 if( require(prada) ) {
     ...
     imageMap(con, "mygraph.jpg", CCbb, tooltips=getNodeNames(gCClo),
              url=rep("", length(nodes(gCC))))
...
\Rpackages{GOstats} they are \Rfunction{simDGGO} and
\Rfunction{simCGO}. The first uses the number of nodes that the two
graphs have in common divided by the total number of nodes in both
graphs combined. Hence these similarities are bounded between 0 and 1
with genes that are more similar having values near one.

For \Rfunction{simCGO} the similarity measure is the depth of the
...
<<multip>>=
 ...
 igenes = unlist(mget(gN[1:10], hgu95av2LOCUSID))

 igenes = igenes[!is.na(igenes)]
 igenes = as.character(igenes)


 GOs = mget(as.character(igenes), GOLOCUSID2GO, ifnotfound=NA)
 gBP = getOntology(GOs, "BP")
 ##how many terms

 sum(sapply(gBP, length))

 ##drop those with no BP annotations
 hasBP = sapply(gBP, function(x) length(x) > 0)

 gBP = gBP[hasBP]

 ggs = lapply(igenes[hasBP], makeGOGraph, "BP")


 sim1 = function(g1, g2)
    length(nodes(intersection(g1, g2)))/length(nodes(join(g1,g2)))
 ...
 for(i in 1:sum(hasBP))
    for( j in 1:sum(hasBP) )
        if( i== j ) next else {
          print(paste(i,j, sep=","))
          simatM2[i,j] = sim2(ggs[[i]], ggs[[j]])
      }

@

Now, since some of our genes have multiple BP mappings we must decide what
to do with them.
