...
%\VignetteDepends{GO, hgu95av2, Biobase, annotate}
...
the concern of the \Rpackage{GO} package.
...
tables provided by the \Rpackage{GO} package.
...
site and the documentation for the \Rpackage{GO} package for a more
...
\item \texttt{GOXXANCESTORS}: the parents, and all their parents and
      so on.
...
The mappings of genes to GO terms is contained in the
\Robject{GOLOCUSID2GO} hash table. This contains mappings from a
LocusLink ID to the most specific applicable GO terms. The hash table,
\Robject{GOLOCUSID2ALLGO} contains the mappings from a LocusLink ID to
all GO terms it is associated with (all parents of the most
specific terms). Each entry in the \Robject{GOLOCUSID2GO} hash table
is a list of lists. The actual entries are named lists, with three
names:
\begin{itemize}
...
\end{itemize}
We note that this same structure is used for the GO mappings in all
chip-based meta-data packages such as \Rpackage{hgu95av2}.
...
In this next example we consider the gene with LocusLink ID
\texttt{25310}. FIXME: what is its symbol etc.
<<locusid>>=

 ll1 = GOLOCUSID2GO[["4121"]]
 length(ll1)
...
by:
%%FIXME: this will change with the new updates from Jianhua
<<sizeofonts>>=

 zz = eapply(GOTERM, function(x) x@Category)
 table(unlist(zz))
...
Finally, we can look at the frequency with which the different
evidence codes are used to map LocusLink Identifiers to GO terms.
...
<<finding these>>=
##FIXME: needs to change when the new GO package is ready
 goterms = unlist(eapply(GOTERM, function(x) x@Ontology))
...
\Rpackage{hgu95av2}. Each of these packages has a data environment
whose name is the name of the package with a \texttt{GO} suffix, so in
this case \Robject{hgu95av2GO}. And these data environments have
exactly the same internal structure as the \Robject{GOLOCUSID2GO}
environment except that the keys are the manufacturers identifiers and
not LocusLink identifiers. Note that if there are many manufacturer
ids that map to the same LocusLink identifier then these will be
duplicate entries (with different keys).

The software tools for working with these packages is then exactly the
same as we described above for working with \Robject{GOLOCUSID2GO}.
