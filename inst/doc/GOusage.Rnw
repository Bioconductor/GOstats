...
\Robject{GOENTREZID2GO} hash table. This contains mappings from a
LocusLink ID to the most specific applicable GO terms.  Each entry in
the \Robject{GOENTREZID2GO} hash table is a list of lists. The actual
entries are named lists, with three names:
\begin{itemize}
...
<<evC>>=
 ...
 LLec = eapply(GOENTREZID2GO, getEvidence)
 table(unlist(LLec))
...
exactly the same internal structure as the \Robject{GOENTREZID2GO}
...
same as we described above for working with \Robject{GOENTREZID2GO}.
